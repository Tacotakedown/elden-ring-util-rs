...
{
    HWND muWindow = NULL;
    std::string muGameName = "ELDEN RING";
    std::string muExpectedWindowName = "ELDEN RING™";
    std::ofstream muLogFile;
    const std::string muAobMask = "?";
    ...
    void Hook(uintptr_t address, uintptr_t destination, size_t extraClearance = 0)
    {
        ...
        *(uintptr_t *)address = 0x0000000025ff;
        ...
        std::cout << "Created jump from " << NumberToHexString(address) << " to " << NumberToHexString(destination) << " with a clearance of " << clearance << std::endl;
    }
}
